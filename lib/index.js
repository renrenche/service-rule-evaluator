 ...
 * @param {String|Number} data 需要检查的值
 * @param {Object} rules 需要匹配的规则
 * @return { info: true | false, msg: string }
 */
const _judge = (data, rules) => {
    // 如果 data 是多个，那个假设 rules 也是多个
    if (data && typeof data === 'object') {
        for (const key in rules) {
            if (_judge(data[key], rules[key]).result === false) {
                return {
                    result: false,
                    msg: ''  //  暂时用不上
                };
        ...
        return {
            result: true,
            msg: `${Object.keys(rules).toString()}`
        };
    }
    ...
    let check = _.every(rules, (threshold, operator) => {
        // 校验规则有效性，可以选择性的忽略无效的规则
        if (typeof VALID_OPERATORS[operator] === 'undefined') {
            if (failOnInvalidRule) {
                debug(`invalid operator ${operator}`);
...
const judge = (data, rules = {}) => {
    // _judge 只判断，然后根据判断结果进行反馈信息
    if (Array.isArray(rules)) {
        let message = '';
        return {
            result: !!_.some(rules, rule => {
                if (_judge(data, rule).result) {
                    message += _judge(data, rule).msg;  // 1、_.some会吞消息 2、some只对正确的处理
                }
                return _judge(data, rule).result;
            }),
            message: message
        };
    } else if (Object.keys(rules).length === 0) {
